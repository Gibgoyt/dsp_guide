...
type GetResponse = Response200 | Response400 | Response401 | Response404 | Response500 | Response503

/*
 * Get specific payment for authenticated user
 *
 * GET /api/users/me/payments/:paymentId
 *
...
export async function GET(accessToken: string, paymentId: string): Promise<GetResponse> {
	try {
		...
		const response = await fetch(
			BASE_URL + `/api/users/me/payments/${paymentId}`, {
				method: "GET",
				headers: {
