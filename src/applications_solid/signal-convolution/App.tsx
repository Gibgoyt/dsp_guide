...
/**
 * Signal Convolution — a SolidJS single-page guide.
 *
 * URL routing (pushState only, no @solidjs/router):
 *   /signal-convolution              -> chapter 1
 *   /signal-convolution/<slug>       -> matching chapter; falls back to chapter 1
 */
const ROOT = 'signal-convolution'
  ...
  return (
    <div class="min-h-screen bg-zinc-950 text-white">
      ...
      <div class="fixed top-0 left-0 right-0 z-50 h-1 bg-zinc-900">
        <div
          class="h-full bg-gradient-to-r from-emerald-400 to-cyan-500 transition-all duration-300"
        ...
        <Show when={isNavOpen()}>
          ...
          <aside class="lg:hidden fixed top-0 left-0 bottom-0 w-72 z-50 bg-zinc-950 border-r border-zinc-800 overflow-y-auto">
            <div class="p-3 flex justify-between items-center border-b border-zinc-800">
              <div class="text-sm font-bold text-emerald-400">Chapters</div>
              <button
                onClick={() => setIsNavOpen(false)}
            ...
            <div class="mt-16 pt-8 border-t border-zinc-800 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Show when={prev()} fallback={<div />}>
                <button
                  ...
                  class="text-left p-4 rounded-xl border border-zinc-800 hover:border-emerald-400 hover:bg-zinc-900/50 transition-all group"
                >
              ...
              <Show when={next()} fallback={<div />}>
                <button
                  ...
                  class="text-right p-4 rounded-xl border border-zinc-800 hover:border-emerald-400 hover:bg-zinc-900/50 transition-all group sm:col-start-2"
                >
          ...
          {(ch, i) => {
            ...
            return (
              <li>
                ...
                >
                  <span class={`font-mono text-xs mt-0.5 w-6 flex-shrink-0 ${isActive() ? 'text-emerald-400' : 'text-zinc-600'}`}>
