use leptos::*;
use crate::api::{create_development_issue, CreateDevelopmentIssueRequest, ProjectStage};
use super::Modal;

#[component]
pub fn CreateDevelopmentIssueModal(
    ...
    let handle_submit = move |ev: ev::SubmitEvent| {
        ...
        let request = CreateDevelopmentIssueRequest {
        ...
        spawn_local(async move {
            match create_development_issue(request).await {
                Ok(issue) => {
                    logging::log!("✅ Development issue created: {}", issue.title);
                    set_loading.set(false);
                    on_close_clone();
                    on_success_clone();
                }
                Err(err) => {
                    logging::error!("❌ Failed to create development issue: {}", err.clone());
    ...
    view! {
        <Modal show=show on_close=on_close title="New Development Issue".to_string()>
