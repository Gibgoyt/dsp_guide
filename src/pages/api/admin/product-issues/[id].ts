...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    ...
    const result = await productIssueQueries.getById(db, id);

    if (!result) {
      return new Response(
        JSON.stringify({ error: 'Product issue not found' }),
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: 'Failed to fetch product issue',
...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    const result = await productIssueQueries.update(db, id, {
      title: data.title,
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: 'Failed to update product issue',
        message: error instanceof Error ? error.message : 'Unknown error',
      }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
